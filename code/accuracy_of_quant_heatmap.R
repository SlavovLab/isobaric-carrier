...
length(unique(ev2$Leading.razor.protein))
length(unique(ev2$Modified.sequence))
...
evxx<-ev2[ (ev2$meanSCRI > scRIq[4] )&(ev2$meanSCRI <= scRIq[5]),  ]
length(unique(evxx$Leading.razor.protein))
length(unique(evxx$Modified.sequence))
...
xx<-melt(ev2[,c("rf","scJU","ms")])
x1<-dcast(ms ~ rf, data =xx, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1])
cor1<-cor(x2, use="complete.obs", method="pearson")

xx<-melt(ev2[,c("rf","scJU","Leading.razor.protein")])
xxb<-aggregate(value ~ Leading.razor.protein + rf, data = xx, median)
x1<-dcast(Leading.razor.protein ~ rf, data =xxb, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1]); dim(x2)
cor1_prot<-cor(x2, use="complete.obs", method="pearson")


xx<-melt(ev2[ev2$meanSCRI>log10(2000),c("rf","scJU","ms")])
x1<-dcast(ms ~ rf, data =xx, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1])
cor1_RI<-cor(x2, use="complete.obs", method="pearson")
...
x1<-dcast(ms ~ rf, data =xx, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1]); dim(x2)
cor2<-cor(x2, use="complete.obs", method="pearson")

xx<-melt(ev2[,c("rf","scJU","Leading.razor.protein")])
xxb<-aggregate(value ~ Leading.razor.protein + rf, data = xx, median)
x1<-dcast(Leading.razor.protein ~ rf, data =xxb, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1]); dim(x2)
cor2_prot<-cor(x2, use="complete.obs", method="pearson")

xx<-melt(ev2[ev2$meanSCRI>log10(2000),c("rf","scJU","ms")])
x1<-dcast(ms ~ rf, data =xx, fill=NA, value.var="value")
x2<-as.matrix(x1[,-1])
cor2_RI<-cor(x2, use="complete.obs", method="pearson")



p1<-ggplot(melt(cor1), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
  scale_x_discrete(labels=c("100","200","300","400","600","800")) + 
  scale_y_discrete(labels=c("100","200","300","400","600","800")) + 
  scale_fill_gradient2(high="red",mid="white",low="blue",na.value="yellow", midpoint=0.5) +
  theme(axis.text.x = element_text(color = "grey20", size = 20, angle = 40, hjust = .5, vjust = .5, face = "plain"),
        axis.text.y = element_text(color = "grey20", size = 20, angle = 0, hjust = 1, vjust = 0, face = "plain"), 
        legend.text = element_text(color = "grey20", size = 12, angle = 0, hjust = 0, vjust = 0.7, face = "plain"), 
        legend.title = element_text(color = "grey20", size = 15, angle = 0, hjust = 0, vjust = 0, face = "plain"), 
        title = element_text(color = "grey20", size = 15, angle = 0, hjust = .5, vjust = 0, face = "plain") )

p2<-ggplot(melt(cor1_RI), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
...
p3<-ggplot(melt(cor2), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
...
p4<-ggplot(melt(cor2_RI), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
...
p5<-ggplot(melt(cor1_prot), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
...
p6<-ggplot(melt(cor2_prot), aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() + 
  theme_bw() + 
  rremove("xylab") +
  labs(fill = "Correlation") + 
...
p5 + p6 + p1 + p2 + p3 + p4 + plot_layout(nrow=3)
